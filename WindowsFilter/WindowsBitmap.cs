...
namespace WindowsFilter
{
    class WindowsBitmap
    {
        ...
        private static unsafe void set_color(byte*color, uint c)
        {
            uint* color_ = (uint*)color;
            color_[0] = c;
        }
        private static unsafe void set_color(byte* color, byte r,byte g,byte b)
        {
            color[1] = r;
            color[2] = g;
            color[3] = b;
        }
        public static unsafe Bitmap unsafe_expand(Bitmap b, int thred = 170, bool IsInvert = false, int dx = 2, int dy=2)
        {
            ...
            for (int i = dx; i < bData.Height; ++i)
            {
                for (int j = dy; j < bData.Width; ++j)
                {
                    ...
                    {
                        set_color(data,front_color);
                        for(int x=0;x<dx;x++)
                            for (int y=0; y<dy; y++)
                            {
                                byte* data_nearby = scan0 + (i-x) * bData.Stride + (j-y) * bytesPerPixel;
                                set_color(data_nearby, front_color);
                            }
                    }
                    else
                    {
                        set_color(data, background_color);
                    }
